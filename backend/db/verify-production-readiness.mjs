...
let versionOk=true;
for(const f of ['package.json','backend/package.json','backend/API_MANIFEST.json']){const x=JSON.parse(await fs.readFile(path.join(root,f),'utf8'));if(x.version!=='249.5.46')versionOk=false;}
checks.push(['version 249.5.46',versionOk]);
...
const report={version:'249.5.46',staticChecks:checks.map(([name,ok])=>({name,ok})),runtimeEnvConfigured:configured,liveTests:'Not run without production credentials/account access'};
