 ...
 const rid=requestId(req); try{
  ...
  if(req.method==='POST'&&String(b.action||'')==='unlock'){
   ...
   try{
    ...
    await client.query(`INSERT INTO creator_earnings(id,creator_id,source_type,source_id,gross_paise,fee_paise,net_paise,status) VALUES($1,$2,'post_unlock',$3,$4,0,$4,'available') ON CONFLICT(source_type,source_id) DO NOTHING`,[id('earn'),post.creator_id,postId,price]);}
